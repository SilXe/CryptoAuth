...
class DowntimeSlasherWrapper extends BaseSlasher_1.BaseSlasher {
    ...
    slashableDowntimeIntervalsBefore(block, maximumLength = 4000) {
        return __awaiter(this, void 0, void 0, function* () {
            const [window, blockchainParamsWrapper, epochManagerWrapper] = yield Promise.all([
                this.getSlashableDowntimeWindow(undefined, block),
                this.contracts.getBlockchainParameters(),
                this.contracts.getEpochManager(),
            ]);
            let end = window.end;
            const intervals = [];
            // TODO(L2): this is deprecated and not supported in L2
            const isL2 = yield (0, connect_1.isCel2)(this.connection.web3);
            while (end > window.start) {
                const epochNumber = isL2
                    ? yield epochManagerWrapper.getEpochNumberOfBlock(end)
                    : yield blockchainParamsWrapper.getEpochNumberOfBlock(end);
                const firstBlock = isL2
                    ? yield epochManagerWrapper.getFirstBlockAtEpoch(epochNumber)
                    : yield blockchainParamsWrapper.getFirstBlockNumberForEpoch(epochNumber);
